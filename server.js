...
app.get('/api/aceite-reporte', async (req, res) => {
  try {
    ...
    const mesNombre = new Date(y, m-1, 1).toLocaleString('es-MX', { month: 'long', year: 'numeric' }).toUpperCase();
    ...
    const result = await db.request()
      ...
      .query(`
        SELECT s.NOM_VENDEDOR AS Vendedor, s.ARTICULO AS NumeroParte, SUM(s.CANTIDAD) AS Cantidad
        FROM FTSABI_PR s
        WHERE s.FECHA >= @ini AND s.FECHA <= @fin
          AND s.ARTICULO IN (${npsLista})
          AND s.NOM_ALMACEN_LIN IN (${SUCURSALES})
    ...
    const filas = Object.entries(porVendedor).map(([vendedor, data]) => {
      const npsVendidos = Object.entries(data.partes).sort((a,b)=>b[1]-a[1]).map(([np,lts])=>`${np} (${Math.round(lts)}L)`).join(', ');
      return {
        Distribuidor: 'CATOSA INTERNATIONAL', GFX: 'zz001', Vendedor: vendedor,
        Mes_Venta: mesNombre, Volumen_Venta: Math.round(data.litrosTotal),
        Litros_International: Math.round(data.litrosIntl), Litros_Fleetrite: Math.round(data.litrosFlrt),
        Numeros_Parte_Vendidos: npsVendidos, Litros_Por_Taller: 0,
      };
...
// ═══════════════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════════════
// ENDPOINTS: PROGRAMA CLIENTES RECUPERADOS — REFACCIONES
// Agregar estos endpoints al server.js existente de Bernardo
// ═══════════════════════════════════════════════════════════════════════════

const fs   = require('fs');
const path = require('path');

// Ruta del archivo JSON donde se guardan los registros del programa
const RECUPERADOS_FILE = path.join(__dirname, 'recuperados_config.json');

// ── Inicializar archivo si no existe ────────────────────────────────────────
function initRecuperadosFile() {
  if (!fs.existsSync(RECUPERADOS_FILE)) {
    fs.writeFileSync(RECUPERADOS_FILE, JSON.stringify({
      config: {
        meses: 60,
        periodo: 'trimestre',
        compraMin: 5000,
        niveles: {
          bronce: { clientes: 1, monto: 10000, premio: 'Smartwatch básico o vale $500 Liverpool' },
          plata:  { clientes: 3, monto: 30000, premio: 'Cafetera Nespresso o auriculares Sony' },
          oro:    { clientes: 5, monto: 60000, premio: 'iPad Air 11" o televisor 50" Smart TV' }
        }
      },
      registros: []
    }, null, 2));
...
function readRecuperados() {
  initRecuperadosFile();
  return JSON.parse(fs.readFileSync(RECUPERADOS_FILE, 'utf8'));
}

function writeRecuperados(data) {
  fs.writeFileSync(RECUPERADOS_FILE, JSON.stringify(data, null, 2));
}

// ── HELPERS SQL ─────────────────────────────────────────────────────────────
const SUCURSALES_SQL = `'REFACCIONES CATOSA','REFACCIONES CATOSA ORIENTE','REFACCIONES CATOSA NORTE','REFACCIONES CATOSA SUR','REFACCIONES CATOSA PONIENTE'`;
const TIPOS_EXCL_SQL = `'PRESUPUESTO','PRESUPUESTO 8%','Traspaso salida almacen'`;

// ═══════════════════════════════════════════════════════════════════════════
// GET /api/recuperados/config
// Devuelve la configuración actual del programa
// ═══════════════════════════════════════════════════════════════════════════
app.get('/api/recuperados/config', (req, res) => {
  try {
    const data = readRecuperados();
...
// Guarda la configuración del programa (solo admin)
// Body: { meses, periodo, compraMin, niveles: { bronce, plata, oro } }
// ═══════════════════════════════════════════════════════════════════════════
app.post('/api/recuperados/config', (req, res) => {
  try {
    const data = readRecuperados();
    data.config = { ...data.config, ...req.body };
    writeRecuperados(data);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Devuelve todos los clientes registrados en el programa
// ═══════════════════════════════════════════════════════════════════════════
app.get('/api/recuperados/registros', (req, res) => {
  try {
    const data = readRecuperados();
...
// Registra un cliente recuperado
// Body: { clienteCodigo, clienteNombre, vendedorCodigo, vendedorNombre,
//         sede, ultimaCompra, fechaRec, monto, notas }
// ═══════════════════════════════════════════════════════════════════════════
app.post('/api/recuperados/registros', (req, res) => {
  try {
    const data = readRecuperados();
    const nuevo = {
      id: Date.now(),
...
// ═══════════════════════════════════════════════════════════════════════════
app.delete('/api/recuperados/registros/:id', (req, res) => {
  try {
    const data = readRecuperados();
    data.registros = data.registros.filter(r => String(r.id) !== String(req.params.id));
    writeRecuperados(data);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Busca clientes en FMCUBI_PR + verifica última compra en FTSABI_PR
// Devuelve: código, nombre, nombre_comercial, última compra, meses inactivo
// ═══════════════════════════════════════════════════════════════════════════
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT TOP 15
          ...
          c.NOMBRE,
          c.NOMBRE_COMERCIAL,
          MAX(LEFT(v.FECHA, 10)) AS ULTIMA_COMPRA
        FROM FMCUBI_PR c
        LEFT JOIN FTSABI_PR v
          ON v.CLIENTE = c.CUENTA
          AND v.NOM_ALMACEN_LIN IN (${SUCURSALES_SQL})
          AND v.DES_TIPO_VENTA NOT IN (${TIPOS_EXCL_SQL})
        WHERE
          c.NOMBRE_COMERCIAL LIKE @q
          OR c.NOMBRE LIKE @q
          OR c.CUENTA LIKE @q
...
// Lista clientes que NO han comprado en X meses — candidatos a recuperar
// Útil para que gerencia vea a quién llamar
// ═══════════════════════════════════════════════════════════════════════════
app.get('/api/recuperados/candidatos', async (req, res) => {
  ...
  // FTSABI_PR.FECHA es varchar YYYYMMDD
  try {
    const pool = await sql.connect(dbConfig);
    const result = await pool.request()
      .query(`
        SELECT TOP 200
          ...
          c.NOMBRE_COMERCIAL,
          c.NOMBRE,
          MAX(LEFT(v.FECHA, 8)) AS ULTIMA_COMPRA_RAW,
          -- Formateado YYYY-MM-DD para el front
          CONCAT(
        ...
        INNER JOIN FTSABI_PR v
          ...
          AND v.NOM_ALMACEN_LIN IN (${SUCURSALES_SQL})
          AND v.DES_TIPO_VENTA NOT IN (${TIPOS_EXCL_SQL})
        GROUP BY c.CUENTA, c.NOMBRE_COMERCIAL, c.NOMBRE
        HAVING MAX(LEFT(v.FECHA, 8)) <= '${fechaStr}'
        ORDER BY ULTIMA_COMPRA_RAW ASC
      `);
    res.json(result.recordset);
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Lista vendedores de refacciones para el selector del formulario
// ═══════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    const result = await pool.request()
      .query(`
        ...
        FROM FMVENBI_PR
        WHERE NOM_ALMACEN_LIN IN (${SUCURSALES_SQL})
...
// Devuelve historial de compras de un cliente en refacciones
// Para verificar inactividad real antes de registrar
// ═══════════════════════════════════════════════════════════════════════════
app.get('/api/recuperados/historial-cliente/:cuenta', async (req, res) => {
  try {
    const pool = await sql.connect(dbConfig);
    const result = await pool.request()
      .input('cuenta', sql.NVarChar, req.params.cuenta)
      .query(`
        SELECT TOP 24
          LEFT(v.FECHA, 4) + '-' + SUBSTRING(v.FECHA,5,2) + '-' + SUBSTRING(v.FECHA,7,2) AS FECHA,
